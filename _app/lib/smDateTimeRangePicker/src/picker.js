...
function Calender($timeout,picker){
	...
	return {
      ...
      scope :{
	   	...
	   	controller:["$scope","$timeout","picker","$mdMedia","$filter",CalenderCtrl],
	    controllerAs : 'vm',
	    templateUrl:/*"lib/smDateTimeRangePicker/dist/picker/calender-date.html",//*/"picker/calender-date.html",
...
var CalenderCtrl = function($scope,$timeout,picker,$mdMedia,$filter){
	var self  = this;
    ...
    self.yearItems = {
		...
		self.monthItems = [];
    for (var m = 0; m <= 11; m++) {
      self.monthItems.push({'month': m, 'texto': $filter('limitTo')(self.monthList[m],3)});
    }

		// console.log(self.monthItems[2].texto);
...
CalenderCtrl.prototype.showYear = function() {
    ...
    self.yearItems.currentIndex_ = (self.initialDate.year() - self.yearItems.START) + 1;
		self.monthTopIndex = self.initialDate.month() -2.8;
};
...
CalenderCtrl.prototype.changeMonth = function(mt){
	var self = this;
	self.initialDate.month(mt);
	self.buildDateCells();
	self.view='DATE';
}
...
function DateTimePicker($mdUtil,$mdMedia,$document,picker){
    return {
      ...
      template: '  <md-input-container md-no-float layout="row">'
                +'<div style="margin-left:-57px; margin-top:-15px; margin-bottom:15px;" hide-sm hide-xs>'
                +'    <md-button class="md-fab md-mini md-primary" aria-label="{{lable}}" ng-focus="show()">'
