...
class read extends conn {
    ...
    /**
     * <b>Exe Read Column:</b> Executa uma leitura simplificada com Prepared Statments. Basta informar o nome da tabela,
     * os termos da seleção (tabelas) e uma analize em cadeia (ParseString) para executar.
     * @param STRING $Tabela = Nome da tabela
     * @param STRING $Colunas= Nome da Coluna ou Colunas
     * @param STRING $ParseString = link={$link}&link2={$link2}
     */
    public function ExeReadColumn($Tabela, $Colunas = null, $ParseString = null) {
        if (!empty($ParseString)):
            parse_str($ParseString, $this->Places);
        endif;

        $this->Select = "SELECT {$Colunas} FROM {$Tabela}";
        $this->Execute();
    }
